...
const definitions: MessageDefinitions = {
  [Locale.EnglishUS]: message`
    ...
    ${quote(`Like what you see and want to know more? You can check all info for a single movie using ${inlineCode('/{{infoCommand}}')}{{#if mentionedRoleId}} or get notified for all future updates like this by using the ${inlineCode('/{{mentionMeCommand}}')} command.{{else}}.{{/if}}`)}
  ...
  [Locale.German]: message`
    ...
    ${quote(`Dir gefällt was du siehst und du willst mehr wissen?. Du kannst alle Info's über einen Film mit ${inlineCode('/{{infoCommand}}')} abrufen{{#if mentionedRoleId}} oder alle zukünftigen Filmupdates mit ${inlineCode('/{{mentionMeCommand}}')} abonnieren.{{else}}.{{/if}}`)}
